...
int main()
{
    ...
    // std::cout << p << std::endl;
    oss.clear();
    assert(x0.setCell(CPos("A0"), "=(1+2*3 ^ 2 ) / (2 < 3) "));
    assert(x0.setCell(CPos("X1"), "=(1 <> 2) + (2 <= 3)  + (2 >= 3) + (4 < 43) + (4 > A2) + (A2 = B2)"));
    assert(x0.setCell(CPos("X2"), "''fr''gt'reg'gr"));
    oss.str("");
    assert(x0.save(oss));
